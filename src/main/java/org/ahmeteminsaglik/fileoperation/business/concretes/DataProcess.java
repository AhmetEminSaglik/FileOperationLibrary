...
public class DataProcess {
    */
/*TODO bu proji libraryye cevirip test edicem.*//*
...
//        String directory = "src\\main\\java\\org\\aes\\searchnode\\fakedata\\DataToTest\\"; //directory For String Data
    public List<FileFundamental> getFileFundementalList(String directory) {
        List<FileFundamental> filePaths = new ArrayList<>();

        try (DirectoryStream<Path> directoryStream = Files.newDirectoryStream(FileSystems.getDefault().getPath(directory))) {
...
//        newFileFund.setFileName("Words-From-Books-English-Readers");
        ...
        for (int i = 0; i < pathList.size(); i++) {
            ...
            if ((i > 0 && i % modFile == 0) || i == (pathList.size() - 1)) {
                newFileFund.setFileName("data-process-test-file");
